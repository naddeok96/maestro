...
def test_mixture_invariance_under_permutation():
    ...
    expanded = torch.cat([encoded, rest.expand(encoded.size(0), -1)], dim=-1)
    policy_heads = PolicyHeads(descriptor_dim=expanded.size(1), context_dim=global_context.size(0))
    mixture, _, _ = policy_heads(expanded, global_context)

    perm = torch.randperm(3)
    summary_perm, encoded_perm = encoder(descriptors[perm])
    global_context_perm = torch.cat([summary_perm, rest], dim=0)
    expanded_perm = torch.cat([encoded_perm, rest.expand(encoded_perm.size(0), -1)], dim=-1)
    mixture_perm, _, _ = policy_heads(expanded_perm, global_context_perm)
